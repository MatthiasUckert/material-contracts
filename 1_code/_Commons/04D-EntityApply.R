...
# 2. Extraction over slices ------------------------------------------------------------------------------------------
# A DEGENERATE CASE, AND DELIBERATELY KEPT. Every label in the settled policy reads full text, so
# ent_slice_plan() emits head rows only -- a tail of zero drops out of the plan -- and a head slice
# of a hundred million characters is the document. The machinery therefore costs nothing today.
#
# It is retained because a window is a filter that can be reimposed, and the whole argument for
# extracting over full text is that the choice stays open. If the corpus pass proves unaffordable, a
# cap is a change to .WINDOW in 04B and nothing here needs rewriting; if it had been deleted, that
# same change would mean restoring an offset-mapping design under time pressure.
...
#'
#' The dispatch in ent_run_engine() is a chain of branches, so a combination it does not cover
#' surfaces as an abort from inside the extraction loop -- after the index is built, the slices are
#' written and the first chunks have run. On a corpus pass that is hours in. Naming the supported
#' set once, here, lets the plan be checked before any work starts.
#'
...
#' engine nothing could run and the rehearsal aborted three combinations into the first chunk.
#'
...
ent_report_throughput <- function(.dir, .n_corpus) {
  ...
  cli::cli_alert_info(
    "Extraction and resolution are timed together, because that is what a pass costs. \\
     CandsPerDoc times the corpus is what 04D's consumer has to store."
  )
  invisible(tab_)
}
