...
cli::cli_h2("The stored column can replace the text read")

# THE POINT OF THE WHOLE CHANGE, TESTED RATHER THAN ASSERTED. mny_load() opens every document to cut
# 60 characters before each amount and test them against .mny_par_cue. If CueBefore carries the same
# language, the read is replaceable -- and at corpus scale that is the difference between a column
# lookup and 1.19 million file opens.
par_in_cue_ <- money_for("doc07") |>
  dplyr::mutate(
    Norm = stringi::stri_trans_toupper(
      stringi::stri_replace_all_regex(.data$CueBefore, "\\s+", " ")
    ),
    Hit  = stringi::stri_detect_fixed(.data$Norm, "PAR VALUE")
  )

chk_on("The par cue is present in CueBefore, so the text read is replaceable", par_in_cue_,
       c("CueBefore", "Hit"), \(.t) any(.t$Hit),
       paste0("...", stringi::stri_sub(par_in_cue_$Norm, from = -34L), " -- mny_load() cuts 60 ",
              "characters for this; the column carries ", .lT$Cue))

# AND THE AFTER SIDE, WHICH NO RULE CAN CURRENTLY REACH. mny_spec()'s roxygen promises "characters
# either side of an amount" and the code cuts a window ending at the span, so par language following
# a figure is invisible -- doc08 is exactly that case and check "Par language AFTER the amount is
# missed" pins it. CueAfter is what makes that fixable, so it has to actually contain the words.
after_ <- money_for("doc08") |>
  dplyr::mutate(
    Norm = stringi::stri_trans_toupper(
      stringi::stri_replace_all_regex(.data$CueAfter, "\\s+", " ")
    ),
    Hit  = stringi::stri_detect_fixed(.data$Norm, "PAR VALUE")
  )

chk_on("And CueAfter carries the par language the current window cannot see", after_,
       c("CueAfter", "Hit"), \(.t) any(.t$Hit),
       paste0(stringi::stri_sub(dplyr::first(after_$Norm), to = 34L),
              "... -- when a rule reads this, doc08's check should flip"))
