...
if (identical(.STAGE, "after")) {
  ...
  compare_ <- function(.a, .b, .key) {
    ...
    a_    <- dplyr::arrange(.a, .data[[.key]])
    b_    <- dplyr::arrange(.b, .data[[.key]])
    ...
    tibble::tibble(
      ...
      ColsRef   = ncol(a_),
      ColsNew   = ncol(b_),
      SameCols  = identical(sort(names(a_)), sort(names(b_))),
      SameKeys  = identical(a_[[.key]], b_[[.key]]),
  ...
  ok_ <- all(res_$SameCols, na.rm = TRUE) &&
    all(res_$SameKeys, na.rm = TRUE) &&
    all(res_$nColsDiff == 0L, na.rm = TRUE) &&
    !any(is.na(res_$RowsNew))

  say_("")
  if (isTRUE(ok_)) {
    say_("Every output reproduces exactly. 01C is verified, and with it the 01 family.")
