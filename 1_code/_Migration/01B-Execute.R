...
#
# THE RESET TOUCHES NOTHING OUTSIDE 01B
# 01B keeps its own mirror root and stages 01A's link tables into it, so everything it produces --
# the staged links, the corpus and the index -- lives under 2_output/01B-EdgarDocuments. Clearing
# that directory and the migration stash is the whole reset. 01A's output is read and never written.
#
...
if (isTRUE(.RESET)) {
  for (d_ in c(dir_01b_, dir_stash_)) {
    if (dir.exists(d_)) {
  ...
  say_("\nClean slate. Nothing outside 01B's own directory was touched.")
} else {
...
lp_edgar_ <- rGetEDGAR::get_directories(file.path(dir_01b_, "GetEDGAR"))
