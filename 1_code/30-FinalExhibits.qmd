---
title: "Final Exhibits -- The Manuscript's Tables and Figures, from the Release (30)"
...
figure note written from the same tibble that drew the figure. `30-Descriptives` was the lab --
every exhibit on every sample, reconciled against both manuscripts -- and was retired on 10
September 2026 once every decision it produced lived here; its readers, vocabulary and reference
numbers are now this document's own.

## One source, and it is hers -- and this document stands alone

Every input is read through this document's own functions: the release `10-ExportData` deploys to
her `MatContractPipeline` folder and the outputs of her `101` and `103` do-files. Nothing that
another document rendered is read, and her `.dta` files are converted under this document's own
cache. What this buys is that a number here and a number in one of her tables come from the same
bytes, and a difference between them is a difference in definition rather than in data -- and that
this document renders on a machine where nothing else ran.
...
the figure's name, and the figure printed here. The data functions behind the figures are the
retired lab's, carried into this library; the runbook only picks the sample and writes the note.
...
fig_redactions_heat_all <- fin_figure_redactions_heat(
  ...
  .period       = "all",
  .min_n        = 30L,                      # cells below this are shaded but not labelled
...
fin_deploy_paper(
  ...
  .run       = TRUE                                        # TRUE deploys; FALSE says what it would do
)
...
The `Where` column of the manifest, once the manuscript / online appendix / memo split is decided
with Ann-Kristin; the appendix figures the specification still names (the within-year timing
figure, redaction by category over time, the ladder-loss figure) and the memo's pandemic panel;
then `3_site` pulls `Output/Figures` and `Output/Tables`.
